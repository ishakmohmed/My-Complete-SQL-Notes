...
FROM customers c
LEFT JOIN orders o
-- this is the fix, means all the records in left table (customers) gets returned regardless if customer has order id or not
	ON c.customer_id = o.customer_id  
-- problem that outer join fixed, like in this case you can't see customers who dont have any order
...
-- (outer is optional btw), now things work the opposite, but if you want it to work like in above code, then swap the order of the tables
	ON c.customer_id = o.customer_id  
ORDER BY c.customer_id
...
-- LEFT JOIN HERE, MEANS YOU WANT ALL RECORDS FROM employees e eeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeee!!!!!!!!!
...
JOIN customers c
	USING (customer_id) 
-- the USING keyword (with parantheses()), because column name is same across both tables, a shortcut!
...
-- the db engine will AUTOMATICALLY combine tables based on columns with same name!
...
-- cross join don't have on statement cause youre mixing the hell out of everything!
...
SELECT 
-- number of col here needa be same with number of col there!!!!!!!!!!!!!!!!!!!!!! SAME WITH DIFFERENT TABLES IN NEXT EXAMPLE!!!!!!!!!!!!!
	order_id,
    order_date,
    'Active' AS status 
    -- THIS IS A STRING LITERAL, newly created col name is status, but value is Active!!!!!!!!!!
FROM orders
...
SELECT first_name  
-- since name clashes, the col in first query gets chosen!
FROM customers
UNION
SELECT name
FROM shippers
