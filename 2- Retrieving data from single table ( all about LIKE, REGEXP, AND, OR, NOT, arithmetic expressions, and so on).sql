...
-- means state = va or state = fl, etc, REMEMBER THE STUFFS ARE SEPARATED BY COMMAS in "IN" !!!
...
--  % means any number of character after b, b could be upper/lowercase
-- ^ you can change b% to brush%, or percentage can come infront too, or front and back

SELECT *
FROM customers
WHERE last_name LIKE '%b%'  
-- means include b, but % means any number of character (opposite in underscore)
...
FROM customers 
WHERE last_name REGEXP '^field' 
-- means must start with field
-- ANOTHER EXAMPLE >> WHERE last_name REGEXP 'field$'
-- means must end with field
...
-- means before e, there should be g or i or m  RIGHT before e
...
-- orrrrrrrrrrrrrrrrr 'br|bu' but initial answer- i guess is better
...
SELECT first_name, last_name, 10 + 1 AS someNumber       
-- 10 + 1 column doesnt exist btw, 
-- ^^^what you're doing is that you add 11 to each record under the newly created column someNumber
FROM customers
ORDER BY someNumber, first_name
-- ORRRRRR ORDER BY 1, 2 so 1 is first column, 2 is second column from SELECT!!!!
...
FROM customers
LIMIT 3 
-- if 300 or something, so obv all will be outputted cause this database table has like 10 records or something
...
LIMIT 3   
-- limit clause should come at the end!!!!