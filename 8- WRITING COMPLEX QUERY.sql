...
-- 	SELECT DISTINCT client_id        -- DISTINCT JUST TO SIMPLIFY THE SUBQUERIES RESULTS
--     FROM invoices
-- )
...
-- SELECT *
-- FROM customers
-- WHERE customer_id IN (       -- when the parantheses opens, it's like you can jump wherever you want, is that right???????
...
-- SELECT *
-- FROM invoices
-- WHERE invoice_total > ALL(            -- in previous implementation, mysql compared each row with max value, now its comparing each row with all multiple invoice total of client with ID 3!
...
-- 	FROM invoices
-- 	GROUP BY client_id
-- 	HAVING COUNT(*) >= 2         -- iirc, having needa refer to cols in SELECT, but I guess COUNT(*) is a general thing so nvm???
-- )
...
-- 	SELECT AVG(salary)
--     FROM employees
--     WHERE office_id = e.office_id        -- if you don't get it, for each employee up there, it'll check this parantheses but only for those which has office IDs as that 1 employee in every iteration!
-- )
...
-- SELECT *
-- FROM clients c
-- WHERE EXISTS (                 -- if you use IN operator, and it returns millions of results, its not really suitable, but EXISTS is because subquery doesn't return result to the outer query, rather it will return an indication whether any row in the subquery matches the search condition (WHERE clause in subquery), if any row meets search criteria, then the subquery returns true to the exists operator!
-- 	SELECT client_id           -- not sure if right or no, but I guess just select stuffs that would be in the where clause
--     FROM invoices
...
-- 	invoice_id,
-- 	invoice_total, 
--     (SELECT AVG(invoice_total) FROM invoices) AS invoice_average,  -- I guess you're using subquery because you want the same invoice average for all records
--     invoice_total - (SELECT invoice_average) AS difference -- note: you cannot use column alias in expression, so either copy the entire expression or SELECT alias name
-- FROM invoices
...
-- 		FROM invoices
-- 		WHERE client_id = c.client_id) AS total_sales,    -- I guess instead of using group by, you're using correlated subquery!
--     (SELECT AVG(invoice_total) FROM invoices) AS average, 
-- 	(SELECT total_sales - average) AS difference    -- you cannot use alias in expression, so you're using the SELECT clause (1 clause for both stuffs and wrap them in parantheses)
-- FROM clients c
...
-- 	FROM clients c
-- ) AS sales_summary -- whenever we use a subquery in a FROM clause, we need to give the subquery an alias, its a MUST!
